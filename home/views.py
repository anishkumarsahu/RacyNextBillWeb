...
def print_invoice(request, *args, **kwargs):
    ...
    pro_list = []
    for pro in saleProduct:
        d_rate = float(pro.rate) - ((float(pro.rate) * float(pro.disc)) / 100.0)
        cA = round((d_rate * (float(pro.gst) / 200)) * pro.quantity, 2)
        sA = round((d_rate * (float(pro.gst) / 200)) * pro.quantity, 2)

        tA = round((round(d_rate * pro.quantity, 2) + cA + sA), 2)
        ...
        pro_dic = {
            'ProAndDis': pro.productName.upper() + des,
            'Hsn': pro.productID.categoryID.hsnID.hsn,
            'Qty': str(pro.quantity) + ' ' + pro.unit,
            'Rate': pro.rate,
            'Disc': pro.disc,
            'TaxableAmount': pro.total,
            'Ctax': float(pro.gst) / 2,
            'Camount': cA,
            'Stax': float(pro.gst) / 2,
            'Samount': sA,
            'Total': tA,
        }
...
def print_bill(request, *args, **kwargs):
    ...
    sale = SalesJet.objects.get(pk=int(query))
    saleProduct = SalesJetProduct.objects.filter(salesID=int(query))
...
@is_activated()
def salesReportJet(request):
    return render(request, 'home/salesReportJet.html')
...
def customer_ledger(request, id=None):
    ...
    paid = 0.0
    total = 0.0 + instance.initialBalance
    for s in sale:
...
@is_activated()
def customer_ledger_jet(request, id=None):
    instance = get_object_or_404(Customer, pk=id)
    sale = SalesJet.objects.filter(customerID_id=instance.pk, isDeleted__exact=False, salesType__icontains='Jet')
    paid = 0.0
    total = 0.0 + instance.initialBalance
    for s in sale:
        paid = paid + s.paidAmount
        total = total + s.grandTotal
    ...
    return render(request, 'home/customerLedgerJet.html', context)
...
def post_customer(request):
    if request.method == 'POST':
        ...
        inBalC = request.POST.get("inBalC")
        ...
        cus.state = cstate
        cus.phone = cphone
        cus.initialBalance = float(inBalC)
        cus.save()
...
def get_customer_detail(request, id=None):
    ...
    data = {
        ...
        'Balance': instance.initialBalance
...
def post_supplier(request):
    if request.method == 'POST':
        ...
        inBalS = request.POST.get("inBalS")
        ...
        sup.state = sstate
        sup.phone = sphone
        sup.initialBalance = float(inBalS)
        sup.save()
        return JsonResponse({'message': 'success'}, safe=False)
...
def edit_supplier(request):
    if request.method == 'POST':
        ...
        cus.state = cstate
        cus.phone = cphone
        cus.initialBalance = float(inBalS)
        cus.save()
        return JsonResponse({'message': 'success'}, safe=False)
...
class CustomerListJson(BaseDatatableView):
    order_columns = ['name', 'gst', 'phone', 'initialBalance', ]
...
class ProductListJson(BaseDatatableView):
    order_columns = ['name', 'brand', 'categoryID', 'mrp',
                     'cost', 'spWithoutGst', 'spWithGst', 'stock', 'warningStockLimit', 'unitID',
                     'discountPc', 'barcode',
                     'status', 'productType'
                     ]
        ...
        if pType == 'Normal':
            ...
            if customerID == 'NaN':
                ...
                for item in splited_receive_item[:-1]:
                    ...
                    pro.save()
                    p.save()
                return JsonResponse({'message': 'success', 'saleID': sale.pk, 'salesType': sale.salesType}, safe=False)
            else:
...
class JetSalesListByProductJson(BaseDatatableView):
    order_columns = ['productName', 'productID.brand', 'salesID.invoiceDate', 'salesID.invoiceNumber',
                     'salesID.customerName', 'salesID.customerGst', 'quantity', 'rate', 'total', 'salesID.salesType',
                     'datetime', ]
    ...
    def get_initial_queryset(self):
        ...
        if 'Admin' in self.request.user.groups.values_list('name', flat=True):
            return SalesJetProduct.objects.filter(salesID__isDeleted__exact=False,
                                                  salesID__invoiceDate__gte=startDate.date(),
                                                  salesID__invoiceDate__lte=endDate.date() + timedelta(days=1))
        else:
            user = CompanyUser.objects.get(user_ID=self.request.user.pk)
...
class SalesListJson(BaseDatatableView):
    order_columns = ['customerName', 'customerGst', 'invoiceDate', 'id', 'invoiceNumber', 'challanNo',
                     'grandTotal', 'paidAgainstBill', 'dueOrReturnAmount', 'paymentType', 'certificateDetail',
                     'datetime', ]
    ...
    def filter_queryset(self, qs):
        ...
        if search:
            qs = qs.filter(
                ...
                | Q(invoiceDate__icontains=search) | Q(invoiceNumber__icontains=search) | Q(challanNo__icontains=search)
                | Q(salesType__icontains=search) | Q(isCertified__icontains=search) | Q(
                    certificateDetail__icontains=search)
    ...
    def prepare_results(self, qs):
        ...
        for item in qs:
            ...
            if 'Admin' in self.request.user.groups.values_list('name', flat=True):
                ...
                action = '''<button style="font-size:10px;" onclick = "CertificateDetail('{}')" class="ui circular  icon button orange">
                               <i class="file contract icon"></i>
                             </button><button style="font-size:10px;" onclick = "TakePayment('{}')" class="ui circular  icon button blue">
...
class JetSalesListJson(BaseDatatableView):
    order_columns = ['customerName', 'customerGst', 'invoiceDate', 'id', 'invoiceNumber', 'challanNo',
                     'grandTotal', 'paidAgainstBill', 'dueOrReturnAmount', 'paymentType',
                     'datetime', ]
    ...
    def get_initial_queryset(self):
        ...
        if 'Admin' in self.request.user.groups.values_list('name', flat=True):
            return SalesJet.objects.filter(isDeleted__exact=False, invoiceDate__gte=startDate.date(),
                                           invoiceDate__lte=endDate.date() + timedelta(days=1), )
        else:
            user = CompanyUser.objects.get(user_ID=self.request.user.pk)
            return SalesJet.objects.filter(isDeleted__exact=False, companyID_id=user.company_ID_id,
                                           invoiceDate__gte=startDate.date(),
                                           invoiceDate__lte=endDate.date() + timedelta(days=1))
    ...
    def prepare_results(self, qs):
        ...
        for item in qs:
            ...
            if 'Admin' in self.request.user.groups.values_list('name', flat=True):
                ...
                if item.isConvertedToNormalSale == True:

                    action = '''<button style="font-size:10px;background-color:#009688;" class="ui circular  icon button">
                               <i class="sync icon"></i>
                             </button><button style="font-size:10px;" onclick = "GetSaleDetail('{}')" class="ui circular  icon button green">
                               <i class="receipt icon"></i>
                             </button>
                    ...
                    action = '''<button style="font-size:10px;" onclick = "convertSale('{}')" class="ui circular  icon button pink">
                                                   <i class="sync icon"></i>
                                                 </button><button style="font-size:10px;" onclick = "GetSaleDetail('{}')" class="ui circular  icon button green">
                                                   <i class="receipt icon"></i>
                                                 </button>
...
class SalesListByCustomerJson(BaseDatatableView):
    order_columns = ['id', 'invoiceDate', 'id', 'invoiceNumber', 'challanNo',
                     'grandTotal', 'paidAgainstBill', 'paymentType', 'datetime', ]
...
class SalesJetListByCustomerJson(BaseDatatableView):
    order_columns = ['id', 'invoiceDate', 'id', 'invoiceNumber', 'challanNo',
                     'grandTotal', 'paidAgainstBill', 'paymentType', 'datetime', ]

    def get_initial_queryset(self):
        if 'Admin' in self.request.user.groups.values_list('name', flat=True):
            return SalesJet.objects.filter(isDeleted__exact=False, customerID_id=self.request.GET.get('ID'))
        else:
            user = CompanyUser.objects.get(user_ID=self.request.user.pk)
            return SalesJet.objects.filter(isDeleted__exact=False, companyID_id=user.company_ID_id,
                                           customerID_id=self.request.GET.get('ID'))
...
def get_sales_detail(request, id=None):
    ...
    basic = {
        ...
        'CertificateDetail': instance.certificateDetail
    }
...
def get_jet_sales_detail(request, id=None):
    instance = get_object_or_404(SalesJet, pk=id)
    basic = {
        'Name': instance.customerName,
        'Gst': instance.customerGst,
        'Phone': instance.customerPhone,
    ...
    items = SalesJetProduct.objects.filter(salesID_id=instance.pk)
    item_list = []
    for i in items:
        item_dic = {
            'ItemID': i.pk,
...
def delete_sales_jet(request):
    if request.method == 'POST':
        id = request.POST.get("ID")
        sale = SalesJet.objects.get(pk=int(id))
        sale.isDeleted = True
        sale.save()
        sales_products = SalesJetProduct.objects.filter(salesID_id=int(id))
        for pro in sales_products:
            product = Product.objects.get(pk=pro.productID_id)
            product.stock = product.stock + pro.quantity
            product.save()
...
def certificate_detail_update(request):
    if request.method == 'POST':
        id = request.POST.get("ID")
        isCertified = request.POST.get("isCertified")
        cDetail = request.POST.get("cDetail")
        sale = Sales.objects.get(pk=int(id))
        sale.isCertified = isCertified
        sale.certificateDetail = cDetail
        sale.save()
...
class ExpenseListJson(BaseDatatableView):
    order_columns = ['expenseType', 'description', 'amount', 'companyID', 'expenseDate', 'datetime']
...
def get_new_sale_invoice_number(request):
    ...
    try:
        lastSale = Sales.objects.filter(invoiceSeriesID_id=int(invoiceSeriesID)).order_by('invoiceActualNumber').last()
    ...
    for item in pur:
        ...
        if len(gsts) > 1:
            worksheet.merge_range('A{}:A{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))), i,
                                  merge_format)
            worksheet.merge_range('B{}:B{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.invoiceDate),
                                  merge_format)
            worksheet.merge_range('C{}:C{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.supplierName),
                                  merge_format)
            worksheet.merge_range('D{}:D{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.supplierGst),
                                  merge_format)
            worksheet.merge_range('E{}:E{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.invoiceNumber),
                                  merge_format)
        else:
            worksheet.write(row - 1, 0, i, merge_format)
    ...
    for item in pur:
        ...
        if len(gsts) > 1:
            ...
            worksheet.merge_range('C{}:C{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  ...
                                  merge_format)
            worksheet.merge_range('D{}:D{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.customerGst),
                                  merge_format)
            worksheet.merge_range('E{}:E{}'.format(str(row - len(gsts) + 1), str(row + len(gsts) - len(gsts))),
                                  str(item.invoiceNumber.replace('&amp;', '&')),
                                  merge_format)
            ...
            worksheet.write(row - 1, 4, str(item.invoiceNumber.replace('&amp;', '&')), merge_format)
...
def JetToNormalSale(request, id=None):
    if request.user.is_authenticated:
        instance = get_object_or_404(SalesJet, pk=id)
        pro = SalesJetProduct.objects.filter(salesID_id=instance.pk)
        ...
        return render(request, 'home/JetToNormalSale.html', context)
    else:
        return redirect('homeApp:loginPage')
...
def add_sales_from_jet_to_normal(request):
    if request.method == 'POST':
        personalDiscount = request.POST.get("personalDiscount")
        jetID = request.POST.get("bookingID")
        customerID = request.POST.get("customerID")
        name = request.POST.get("name")
        gst = request.POST.get("gst")
        phone = request.POST.get("phone")
        ...
        if customerID == 'NaN':
            ...
            for item in splited_receive_item[:-1]:
                ...
                # pro.save()
                p.save()
            book = SalesJet.objects.get(pk=int(jetID))
            book.isDeleted = True
            book.save()
            return JsonResponse({'message': 'success', 'saleID': sale.pk}, safe=False)
        else:
            ...
            for item in splited_receive_item[:-1]:
                ...
                p.save()
            book = SalesJet.objects.get(pk=int(jetID))
            book.isConvertedToNormalSale = True
            book.save()
            return JsonResponse({'message': 'success', 'saleID': sale.pk}, safe=False)
