...
class SalesJetProductListAdmin(admin.TabularInline):
    model = SalesJetProduct
    extra = 0




class JetSalesAdmin(admin.ModelAdmin):
    search_fields = ['customerName','customerGst','invoiceNumber']
    list_display = ['customerName','isConvertedToNormalSale','customerGst','invoiceNumber','invoiceDate','grandTotal','companyID','status', 'isDeleted', 'datetime', 'lastUpdatedOn']

    inlines = (SalesJetProductListAdmin,)
admin.site.register(SalesJet, JetSalesAdmin)